<template>
  <div>
    <div style="display: flex; justify-content: flex-end; margin-bottom: 16px">
      <n-button type="primary" @click="openCreateModal">+ Создать тариф</n-button>
    </div>

    <n-data-table :columns="columns" :data="tariffs" :bordered="false" striped />

    <n-modal v-model:show="showModal" :title="editingItem ? 'Редактирование тарифа' : 'Создание тарифа'" preset="card" style="max-width: 520px" :mask-closable="false">
      <n-form ref="modalFormRef" :model="modalForm" :rules="modalRules" label-placement="top">
        ...
        <n-form-item label="Описание" path="description">
          <n-input v-model:value="modalForm.description" type="textarea" placeholder="Описание тарифа" :rows="2" />
        </n-form-item>
        <n-form-item label="Цвет" path="color">
          <n-color-picker v-model:value="modalForm.color" :swatches="colorSwatches" :modes="['hex']" />
        </n-form-item>
...
const columns = [
  {
    ...
    render(row) {
      return h('div', { style: 'display: flex; align-items: center; gap: 8px' }, [
        h('span', { style: { width: '12px', height: '12px', borderRadius: '50%', display: 'inline-block', background: row.color, flexShrink: 0 } }),
